...
function SacolaModal({ closeModal }) {
	...
	const getListas = async () => {
		...
		if(Array.isArray(sacolaLista)) {
			const novaLista = sacolaLista.map(
				({paletaId, quantidade}) => ({ nome: encontraNome(paletaId), quantidade })
			)
  ...
  return (
    <Modal closeModal={handleClose}> 
			<div className="SacolaModal">
				...
				<div>
					<button onClick={purchase} className="SacolaModal__confirmar"> Fechar compra </button>
				</div>
			</div>
    </Modal>
  );
